  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item job \texttt{sbatch} exclusivo; compilação no nó (\texttt{-O3 -march=native -fopenmp}); governor \texttt{performance}.
      \end{itemize}
  \end{columns}
  \vfill
...
\begin{frame}{Diagnóstico do gargalo}
  \begin{itemize}
    \item Pico teórico do nó $\approx$ 736 GFLOP/s; a v0 atinge \textbf{0,94 GFLOP/s $\approx$ 0,1\%} $\rightarrow$ o gargalo \textbf{não} é cômputo.
  ...
  \begin{block}{Hipóteses de intervenção}
    \begin{enumerate}
      \item[H1.] Vetorizar o produto escalar (\texttt{omp simd}). \hfill \alert{(falsa)}
      \item[H2.] Paralelizar o laço externo com granularidade grossa. \hfill (verdadeira)
      \item[H3.] Trocar a ordem dos laços para eliminar o stride --- a causa raiz. \hfill (verdadeira)
    \end{enumerate}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]{v2 e v3 --- a MESMA técnica, granularidades opostas}
  \begin{columns}[T]
    \column{0.48\textwidth}
      \textbf{v2: laço externo (grossa) $\rightarrow$ 17,5$\times$}
\begin{semiverbatim}\small
...
\end{semiverbatim}
      ...
      \end{itemize}
    \column{0.48\textwidth}
      \textbf{v3: laço interno (fina) $\rightarrow$ \alert{0,23$\times$}}
\begin{semiverbatim}\small
for (i ...) for (j ...) \{
#pragma omp parallel for \\
        reduction(+:sum)
...
\begin{frame}{Resultados (hype, $N=1024$, mediana de 5 repetições)}
  ...
  \end{table}
  \begin{itemize}
    \item Extra com Hyper-Threading (40 threads): v2 $+4\%$; v4 $+38\%$ (HT esconde latência de memória).
...
\begin{frame}{Discussão: por que cada resultado aconteceu}
  \begin{itemize}
    \item \textbf{v1 (0,96$\times$):} SIMD multiplica cômputo, mas o tempo é gasto esperando memória. \emph{Uma técnica só é boa otimização quando resolve o gargalo dominante.}
...
\begin{frame}{Conclusão}
  \begin{block}{Melhor configuração}
    \textbf{v4} --- i-k-j + \texttt{omp parallel for}, 20 threads: \textbf{0,017 s, 132$\times$} sobre o baseline, resultado bit a bit idêntico.
  \end{block}
  \begin{block}{Principal aprendizado técnico}
    \textbf{O valor de uma técnica depende do gargalo, não da técnica}: 0,96$\times$, 0,23$\times$, 17,5$\times$ e 132$\times$ --- todas construções OpenMP sobre o mesmo kernel.
  \end{block}
  \begin{itemize}
    \item Limitações: sem contadores de hardware no ambiente; first-touch NUMA limita o teto; v4 usa $\approx$ 17\% do pico (próximo passo: tiling).
  \end{itemize}
  \vfill
  \footnotesize Código, job SLURM, dados e gráficos: \texttt{github.com/Spolavore/hpc-final} \\
  Some experiments in this work used the PCAD infrastructure, \texttt{http://pcad.inf.ufrgs.br}, at INF/UFRGS.
\end{frame}
